...
public class Estudante extends Pessoa {
    ...
    public void consolidarNota(int unidade, double nota) {
        notas[unidade - 1] = nota; // -1 é para garantir que vai iniciar no índice 0
    }
    ...
    public String verSituacao(int mes) {
        ...
        if(getNotas()[unidade - 1] >= 7.0) {
            return "O aluno está na média na unidade " + unidade + " e obteve a nota " + getNotas()[unidade - 1];
        }
        return "O aluno não está na média na unidade " + unidade + " e obteve a nota " + getNotas()[unidade - 1];
    }
    @Override
